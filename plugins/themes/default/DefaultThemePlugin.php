...
class DefaultThemePlugin extends \PKP\plugins\ThemePlugin
{
    ...
    public function init()
    {
        ...
        if ($context && ($homepageImage = $context->getLocalizedData('homepageImage'))) {
            if (is_string($homepageImage)) {
                $homepageImage = json_decode($homepageImage, true);
            }
            if ($homepageImage && isset($homepageImage['uploadName'])) {
                $publicFileManager = new PublicFileManager();
                $publicFilesDir = $request->getBaseUrl() . '/' . $publicFileManager->getContextFilesPath($context->getId());
                $homepageImageUrl = $publicFilesDir . '/' . $homepageImage['uploadName'];

                // If the option is set to use the homepage image as a header, add an inline style
                if ($this->getOption('useHomepageImageAsHeader')) {
                    $this->addStyle(
                        'homepageImage',
                        '.journal_hero { background-image: url(' . $homepageImageUrl . '); }',
                        ['inline' => true]
                    );
                }
            }
